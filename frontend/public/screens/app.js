 appScript = `
    ...
    window.balancePageRender = function (dataObject) {
        ...
        $('#fund_wallet_card_number').on('keyup', function(e){
            ...
            window.addWithdrawal = function(){
                ...
                */
                ...
                "WithdrawalResponse","Withdrawal successfully processed to repient Bank");
            }
        ...
        window.submitMorgageLoan = function (){
            ...
            window.checkCardAval =  function (){
                if(document.getElementById("usd_card_sel").checked == true || document.getElementById("ng_card_sel").checked == true){
                    if(document.getElementById("ng_card_sel").checked == true){
                        swal('Sorry','Only USD cards are available at the moment, kindly check back later','warning');
                        return false;
                    }                    
                    if(document.getElementById("usd_card_sel").checked == true){
                        var checkCardAvalApi = function (){
                            show_loading1();
                            /* call car checking api here */
                            window.cardCheck = false;
                            w3.getHttpObject(appData.root_url + "api/check_card_aval/" + user_info.member_id,  function (dataObject) {
                                if(dataObject.allowed == true){
                                    window.cardCheck =  true;
                                    document.getElementsByClassName("show-invest-begin-doc2")[0].click();
                                    hide_loading1();
                                    /* Initiate card creation */
                                    w3.getHttpObject(appData.root_url + "api/create_card_usd/" + user_info.member_id,  function (dataObject) {
                                        /*Get Card info */
                                        getCard();
                                    });
                                    return true;
                                    ...
                                    window.cardCheck =  false;
                                    swal('Sorry','Due to hgih demand, Yobe Mfb USD Cards and NGN Cards are currently out of stock. Kindly cehck back soon.','warning');                                    
                                    hide_loading1();
                                    return false;
                                }
                            });
        ...
        window.getCard = function (){
            /* Get card info */
            w3.getHttpObject(appData.root_url + "api/get_card/" + user_info.member_id, function(dataObject){
                if(dataObject.id){
                    user_info.has_usd_card = true;
                    user_info.usd_card = dataObject;
                    $(".card_number").html(dataObject.card_number);
                    $(".expiration_month").html(dataObject.expiration_month);
                    $(".exiration_year").html(dataObject.exiration_year);
                    $(".cvv").html(dataObject.cvv);
                }else{
                    user_info.has_usd_card = false;
                }
            });

            /* Get usd balance */
            w3.getHttpObject(appData.root_url + "api/usd_balance/" + user_info.member_id, function(dataObject){
                if(dataObject){
                    window.usd_balance = dataObject.usd_balance;
                    $(".usd_balance").html(toDP(dataObject.usd_balance.toLocaleString('en-US', 'numeric')));
                    if(dataObject.usd_balance > 0.01){
                        $("#usd_withdraw div").removeClass("text-grey");
                        $("#usd_withdraw button").removeClass("grey");
                        $("#usd_withdraw button").addClass("navy");
                        $("#usd_withdraw").addClass("show-withdraw-usd");
                    }else{
                        $("#usd_withdraw div").addClass("text-grey");
                        $("#usd_withdraw button").addClass("grey");
                        $("#usd_withdraw button").removeClass("navy");
                        $("#usd_withdraw").removeClass("show-withdraw-usd"); 
                        $("#usd_withdraw").attr("onclick", "return false"); 

                    }
                }else{
                    swal("An error occured getting USD balance");
                }
            });
                    ...
                    window.usdControl = function (){
                        if(appData.balance_page.total_balance < appData.settings.usd_minimum){
                            withdrawal_message = appData.settings.usd_submmited.replaceAll("@amount", "₦" + Number(appData.settings.usd_minimum).toLocaleString('en-US', 'numeric'));
                            swal('Failed',withdrawal_message,'error');
                        }else{
                            swal('Processing',appData.settings.withdrawal_proccessing,'warning');        
                        }
                    ...
                    window.addusd = function(){
                        if((Number($("#ngn_value").val().replace(",","")) *  appData.settings.usd_rate) > Number(appData.balance_page.total_balance)){
                            swal('Insufficient Fund!',"Note: Minimum Monthly spend Limit is $" + appData.settings.usd_spend_limit,'error');  
                            return false;
                        }
                        
                        if((Number($("#ngn_value").val().replace(",","")) *  appData.settings.usd_rate)  < appData.settings.usd_minimum_deposit){
                            /*smallBottomFade("Please enter amount lesser than your wallet balance ₦" + Number(appData.balance_page.total_balance).toLocaleString('en-US', 'numeric'),"white","red");*/
                            /*smallBottomFade("Minimum Monthly spend Limit is $" + appData.settings.usd_spend_limit,"white","red"); */
                            swal('Insufficient Fund!',"Note: Minimum Monthly spend Limit is $" + appData.settings.usd_spend_limit,'error');  
                            return false;
                        }
                                       
                        var payload = {
                            title : 'USD CARD FUNDING',
                            member_id : user_info.member_id,
                            amount: $("#ngn_value").val(),
                            account: "",
                            description: ""
                        }
                        show_loading1();
                        reqAsyncJSON("POST", serverHost + api_prefix + "api/wallet/request_withdrawal", payload, 
                        "usdResponse","Withdrawal request");
                    }
                
                
                    window.usdResponse = function (res){
                        console.log(res);
                        if(JSON.parse(res[0]).success != true){
                            hide_loading1();
                            smallBottomFade(JSON.parse(res[0]).message,"white","red");
                        ...
                        /*swal("Successful",JSON.parse(res[0]).message,"success");*/
                        $(".widthdrawal-header").html("");
                        $(".widthdrawal-body").html(appData.settings.usd_fund_response);
                        $(".show-widthdrawal-response").click();    
                    ...
                    window.subusd = function(){
                        if(Number($("#withdraw_amount_usd").val().replace(",","")) < 2){
                            swal('Wrong Input',"Minimum amount is $2",'error');  
                            return false;
                        }
                        if(Number($("#withdraw_amount_usd").val().replace(",","")) > window.usd_balance){
                            swal('Oops',"Insifficient fund",'error');  
                            return false;
                        }
                        ...
                        var payload = {
                            title : 'USD CARD FUNDING',
                            member_id : user_info.member_id,
                            amount: '-' + ($("#withdraw_amount_usd").val().replace(",","") * appData.settings.usd_rate),
                            account: "USD Withdrawal",
                            description: ""
                        }
                        show_loading1();
                        reqAsyncJSON("POST", serverHost + api_prefix + "api/wallet/request_withdrawal", payload, 
                        "usdSubResponse","Withdrawal request");
                    }
                
                
                    window.usdSubResponse = function (res){
                        console.log(res);
                        if(JSON.parse(res[0]).success != true){
                            hide_loading1();
                            smallBottomFade(JSON.parse(res[0]).message,"white","red");
                ...
                window.tv_logo = { "DSTV":"images/networks/dstv.jpg",
                                        "GOTV":"images/networks/gotv.jpg"
                                        };
                ...
                window.tvPlans = { "DSTV":["160+ Channels @ ₦21,000/1 MONTH","145+ Channels @ ₦14,250/1 MONTH","140+ Channels @ ₦12,000/1 MONTH","130+ Channels @ ₦9,000/1 MONTH","105+ Channels @ ₦5,300/1 MONTH","85+ Channels @ ₦2,950/1 MONTH","45+ Channels @ ₦2,150/1 MONTH"],
                                    "GOTV":["80+ Channels @ ₦8,500/1 MONTH","75+ Channels @ ₦4,150/1 MONTH","65+ Channels @ ₦2,800/1 MONTH","45+ Channels @ ₦1,900/1 MONTH","35+ Channels @ ₦900/1 MONTH"],
                                };
                ...
                window.initData = function (network){
                    ...
                    var options = "";
                        ...
                        for(var x =0; x<plans.length; x++){
                            /*50MB @ ₦50/1 DAY*/
                            var amount = plans[x].split("₦")[1].split("/")[0].split(",").join("");
                ...
                window.initTV = function (network){
                    $(".network-icon").attr("src", tv_logo[network]);
                    $("#pay_bill_tv_service_provider").val(network);                    
                    var plans = tvPlans[network];
                    var options = "";
                        options += "<option value=''>--CHOOSE PLAN--</option>";
                        for(var x =0; x<plans.length; x++){
                            /*50MB @ ₦50/1 DAY*/
                    ...
                    $("#pay_bill_tv_service_amount").html(options);
                
                };
                window.doTV =  function(){             
                    var member_id =  user_info.member_id;
                    var service_type = "TV";
                    var service_provider = $("#pay_bill_tv_service_provider").val();
                    var service_number = $("#pay_bill_tv_service_number").val();
                    var service_amount = $("#pay_bill_tv_service_amount").val();
                    var service_customer = $("#pay_bill_airtime_service_name").val();
                    var service_note = $("#pay_bill_tv_service_note").val();

                    if(service_amount.trim() == ""){
                        smallBottomFade("Please select a tv package","white","red");
                        return false;
                    }

                    if(service_number.trim() == ""){
                        smallBottomFade("Please enter card number","white","red");
                        return false;
                    } 
                    
                    if(service_number.trim().length < 4){
                        smallBottomFade("Please enter a valid smart card number","white","red");
                        return false;
                    }                     
            ...
            window.logout = function (){
                ...
                $(".show-login").click();
                    ...
                    window.snap = function(){
                        /*alert("Please ensure good lighting and clear background");*/
                        try {
                            window.navigator.camera.getPicture(onSuccess, onFail, { quality: 25,
                                destinationType: Camera.DestinationType.DATA_URL, cameraDirection: Camera.Direction.FRONT
                            });  
                         }
                        catch(err) {
                            alert(err.message);
                ...
                window.signup_otp = 0;
                window.OTP = function(){
                    signupObj = signupObjInit();
                    var signup_phone =  $("#signup_phone").val();
                    window.signup_otp = Math.ceil(Math.random() * 8999) + 1000;
                    reqAsyncJSON('POST', serverHost + api_prefix + "api/verify_member/" + signup_otp, signupObj, 
                ...
                window.OTPResponse = function (cookie){
                    ...
                    if(JSON.parse(cookie[0]).success == true){
                        alert(JSON.parse(cookie[0]).message);
                    }
                    
                    myPrompt = prompt("Enter 4-digit code sent to your phone and email:");
                    if(myPrompt == window.signup_otp){
                        swal("Successful", "Your contact is validated", "success");
                        window.bvnValidated = true;
                        $("#signup_transaction_pin").val(myPrompt);
                        $("#signup_transaction_pin").attr("disabled","disabled");
                        $("#verify_bvn").val("Verified");
                        $("#verify_bvn").attr("disabled","disabled");
                    }else{
                        swal("Failed", "invalid Code, try again", "error");
                        $("#signup_transaction_pin").val("");
                        $("#verify_bvn").val("Resend code");
                    }
                }
                ...
                window.signupObj = signupObjInit();
                window.signUp = function (){
                            ...
                            if(bvnValidated == true){
                                ...
                                swal("PHONE NUMBER OR EMAIL NOT VALIDATED");
                                return false;
                            }
                ...
                window.validate_signup_1 = function (){
                  /**/
                  signupObj = signupObjInit();
 ...
 if(signupObj.signup_employment_status.trim() == ""){
                  ...
                  if(signupObj.signup_state_of_origin.trim() == ""){
                        ...
                        document.getElementsByClassName("show-signup")[0].click();
                        swal("Next of kin name is Reqired!");
                        return false;
                    }                     
                                                                                            if(signupObj.signup_guarantor_phone1.trim() == ""){
                        document.getElementsByClassName("show-signup")[0].click();
                        swal("Next of kin Phone Number is Reqired!");
                        return false;
                    }                     
                        ...
                        document.getElementsByClassName("show-signup")[0].click();
                        swal("Next of kin phone is Reqired!");
                        return false;
                    }                     
                                                                                            if(signupObj.signup_guarantor_relationship2.trim() == ""){
                        document.getElementsByClassName("show-signup")[0].click();
                        swal("Next of kin relationship is Reqired!");
                        return false;
                    }                     
  /**/
              ...
              if(signupObj.agreeterms == true){
                        ...
                        document.getElementsByClassName("show-signup")[0].click();               
               swal("Accept Terms and Conditions to continue");
               return false;
             }     
                ...
                window.validate_signup = function (){
                  /**/
                  signupObj = signupObjInit();                  
              ...
              if(signupObj.signup_password != signupObj.signup_password2){
               ...
               if(signupObj.signup_bvn.length != 11){
                    ...
                    /* VALIDATE PHOTO CAPTURE AND EMAIL*/
                if(window.isCaptured == false) {
                    swal("Image not captured yet!");
                    return false;
                }

                if(signupObj.agreeterms2 == true){
